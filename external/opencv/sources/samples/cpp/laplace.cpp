...
int main( int argc, char** argv )
{
    ...
    if (camera.size() == 1 && isdigit(camera[0]))
        m_cap.open(parser.get<int>("c"));
    else
        m_cap.open(samples::findFileOrKeep(camera));
    if (!m_cap.isOpened())
    {
    ...
    cout << "Video " << parser.get<string>("c") <<
        ": width=" << m_cap.get(CAP_PROP_FRAME_WIDTH) <<
        ", height=" << m_cap.get(CAP_PROP_FRAME_HEIGHT) <<
        ", nframes=" << m_cap.get(CAP_PROP_FRAME_COUNT) << endl;
    int pos = 0;
    if (parser.has("p"))
    {
    ...
    if (pos != 0)
    {
        ...
        if (!m_cap.set(CAP_PROP_POS_FRAMES, pos))
        {
