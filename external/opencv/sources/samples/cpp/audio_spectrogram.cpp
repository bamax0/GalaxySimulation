...
public:
    ...
    int readAudioFile(string file, vector<int>& inputAudio)
    {
        ...
        m_cap.open(file, CAP_ANY, params);
        if (!m_cap.isOpened())
        {
        ...
        int samplingRate =  static_cast<int>(m_cap.get(CAP_PROP_AUDIO_SAMPLES_PER_SECOND));
        ...
        for (;;)
        {
            if (m_cap.grab())
            {
                m_cap.retrieve(frame, audioBaseIndex);
                frameVec = frame;
    ...
    int readAudioMicrophone(vector<int>& inputAudio)
    {
        ...
        const int audioBaseIndex =  static_cast<int>(m_cap.get(CAP_PROP_AUDIO_BASE_INDEX));
        const int numberOfChannels =  static_cast<int>(m_cap.get(CAP_PROP_AUDIO_TOTAL_CHANNELS));
        cout << "CAP_PROP_AUDIO_DATA_DEPTH: " << depthToString( static_cast<int>(m_cap.get(CAP_PROP_AUDIO_DATA_DEPTH))) << endl;
        int samplingRate = static_cast<int>(m_cap.get(CAP_PROP_AUDIO_SAMPLES_PER_SECOND));
