...
int main(int argc, char *argv[])
{
    ...
    TickMeter total;
    Ptr<VideoCapture> m_cap;
    Ptr<VideoWriter> wrt;
    try
    {
        if (mode == "decode")
        {
            m_cap = createCapture(backend, file_name, codec);
            if (!m_cap)
            {
        ...
        else if (mode == "encode")
        {
            ...
            m_cap = createSynthSource(sz, fix_fps);
            ...
            if (!m_cap->isOpened() || !wrt->isOpened())
            {
    ...
    while(true)
    {
        ...
        {
            int limit = 100;
            while (!m_cap->grab() && --limit != 0)
            {
                cout << "Skipping empty input frame - " << limit << endl;
            }
            m_cap->retrieve(element);
            tick.start();
            *wrt << element;
            tick.stop();
        }
