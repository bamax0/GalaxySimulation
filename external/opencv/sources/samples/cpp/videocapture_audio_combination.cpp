...
int main(int argc, char** argv)
{
    ...
    m_cap.open(file, CAP_MSMF, params);
    if (!m_cap.isOpened())
    {
    ...
    cout << "CAP_PROP_AUDIO_TOTAL_CHANNELS: " << m_cap.get(CAP_PROP_AUDIO_TOTAL_CHANNELS) << endl;
    ...
    for (;;)
    {
        if (m_cap.grab())
        {
            m_cap.retrieve(videoFrame);
            for (int nCh = 0; nCh < numberOfChannels; nCh++)
            {
                m_cap.retrieve(audioFrame, audioBaseIndex+nCh);
                if (!audioFrame.empty())
