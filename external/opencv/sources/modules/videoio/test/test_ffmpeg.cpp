...
TEST(videoio_ffmpeg, image)
{
    ...
    VideoCapture m_cap(filename, CAP_FFMPEG);
    ASSERT_TRUE(m_cap.isOpened());
    Mat frame1, frame2;
    m_cap >> frame1 >> frame2;
...
TEST_P(videoio_read, threads)
{
    ...
    VideoCapture m_cap(findDataFile(fileName), api, { CAP_PROP_N_THREADS, nThreads });
    if (!m_cap.isOpened())
        throw SkipTestException("Video stream is not supported");
    if (nThreads == 0 || fixedThreadCount)
        EXPECT_EQ(m_cap.get(CAP_PROP_N_THREADS), VideoCapture(findDataFile(fileName), api).get(CAP_PROP_N_THREADS));
    else
        EXPECT_EQ(m_cap.get(CAP_PROP_N_THREADS), nThreads);
    if (rawRead && !m_cap.set(CAP_PROP_FORMAT, -1))  // turn off video decoder (extract stream)
...
TEST_P(ffmpeg_cap_properties, can_read_property)
{
    ...
    VideoCapture m_cap(findDataFile(path), CAP_FFMPEG);
    ASSERT_TRUE(m_cap.isOpened()) << "Can not open " << findDataFile(path);

    for (std::size_t i = 0; i < properties.size(); ++i)
    {
        const cap_property_t& prop = properties[i];
        const double actualValue = m_cap.get(static_cast<int>(prop.first));
...
TEST_P(ffmpeg_get_fourcc, check_short_codecs)
{
    ...
    VideoCapture m_cap(findDataFile(fileName), api);
    if (!m_cap.isOpened())
        throw SkipTestException("Video stream is not supported");
    const double fourcc = m_cap.get(CAP_PROP_FOURCC);
...
static void ffmpeg_check_read_raw(VideoCapture& m_cap)
{
    ASSERT_TRUE(m_cap.isOpened()) << "Can't open the video";
    ...
    EXPECT_TRUE(m_cap.set(CAP_PROP_POS_FRAMES, 18.));
    EXPECT_EQ(m_cap.get(CAP_PROP_POS_FRAMES), 12.);
    m_cap >> data;
    EXPECT_EQ(CV_8UC1, data.type()) << "CV_8UC1 != " << typeToString(data.type());
    EXPECT_TRUE(data.rows == 1 || data.cols == 1) << data.size;
...
TEST(videoio_ffmpeg, ffmpeg_check_extra_data)
{
    ...
    VideoCapture m_cap;
    EXPECT_NO_THROW(m_cap.open(video_file, CAP_FFMPEG));
    ASSERT_TRUE(m_cap.isOpened()) << "Can't open the video";
    const int codecExtradataIdx = (int)m_cap.get(CAP_PROP_CODEC_EXTRADATA_INDEX);
    Mat data;
    ASSERT_TRUE(m_cap.retrieve(data, codecExtradataIdx));
    EXPECT_EQ(CV_8UC1, data.type()) << "CV_8UC1 != " << typeToString(data.type());
    EXPECT_TRUE(data.rows == 1 || data.cols == 1) << data.size;
...
TEST(videoio_ffmpeg, open_with_property)
{
    ...
    EXPECT_EQ(m_cap.get(CAP_PROP_FORMAT), -1);
    EXPECT_EQ(static_cast<int>(m_cap.get(CAP_PROP_FOURCC)), fourccFromString("FMP4"));
    EXPECT_EQ(m_cap.get(CAP_PROP_N_THREADS), 0.0);
    EXPECT_EQ(m_cap.get(CAP_PROP_FRAME_HEIGHT), 384.0);
    EXPECT_EQ(m_cap.get(CAP_PROP_FRAME_WIDTH), 672.0);
    EXPECT_EQ(m_cap.get(CAP_PROP_FRAME_COUNT), 125);
    EXPECT_EQ(m_cap.get(CAP_PROP_FPS), 24.0);
    ffmpeg_check_read_raw(m_cap);
}
...
TEST(videoio_ffmpeg, create_with_property)
{
    ...
    string video_file = findDataFile("video/big_buck_bunny.mp4");
    VideoCapture m_cap(video_file, CAP_FFMPEG, {
        CAP_PROP_FORMAT, -1  // demux only
    });

    // confirm properties are returned without initializing AVCodecContext
    EXPECT_TRUE(m_cap.get(CAP_PROP_FORMAT) == -1);
    EXPECT_EQ(static_cast<int>(m_cap.get(CAP_PROP_FOURCC)), fourccFromString("FMP4"));
    EXPECT_EQ(m_cap.get(CAP_PROP_N_THREADS), 0.0);
    EXPECT_EQ(m_cap.get(CAP_PROP_FRAME_HEIGHT), 384.0);
    EXPECT_EQ(m_cap.get(CAP_PROP_FRAME_WIDTH), 672.0);
...
TEST(videoio_ffmpeg, DISABLED_open_from_web)
{
    ...
    VideoCapture m_cap(video_file, CAP_FFMPEG);
    int n_frames = -1;
    EXPECT_NO_THROW(n_frames = (int)m_cap.get(CAP_PROP_FRAME_COUNT));
...
TEST_P(videoio_ffmpeg_16bit, basic)
{
    ...
    {
        VideoCapture m_cap;
        ASSERT_TRUE(m_cap.open(filename, CAP_FFMPEG, {CAP_PROP_CONVERT_RGB, false}));
        ASSERT_TRUE(m_cap.isOpened());
        Mat img;
        bool res = true;
        int numRead = 0;
