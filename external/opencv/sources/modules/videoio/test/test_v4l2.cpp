...
TEST_P(videoio_v4l2, formats)
{
...

    {
        ...
        VideoCapture m_cap;
        ASSERT_TRUE(m_cap.open(device, CAP_V4L2));
        // VideoCapture will set device's format automatically, vivid device will accept it
        ASSERT_TRUE(m_cap.set(CAP_PROP_FOURCC, params.pixel_format));
        ASSERT_TRUE(m_cap.set(CAP_PROP_CONVERT_RGB, false));
        for (size_t idx = 0; idx < 3; ++idx)
        {
            Mat img;
            EXPECT_TRUE(m_cap.grab());
            EXPECT_TRUE(m_cap.retrieve(img));
