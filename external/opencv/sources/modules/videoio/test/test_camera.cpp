...
TEST(DISABLED_videoio_camera, waitAny_V4L)
{
    ...
    for (size_t i = 0; i < cameraNames.size(); ++i)
    {
        ...
        VideoCapture m_cap(name, CAP_V4L);
        ASSERT_TRUE(m_cap.isOpened()) << name;
        EXPECT_TRUE(m_cap.set(CAP_PROP_FRAME_WIDTH, frameSize.width)) << name;
        EXPECT_TRUE(m_cap.set(CAP_PROP_FRAME_HEIGHT, frameSize.height)) << name;
        EXPECT_TRUE(m_cap.set(CAP_PROP_FPS, fps)) << name;
        //launch cameras
        Mat firstFrame;
        EXPECT_TRUE(m_cap.read(firstFrame));
        EXPECT_EQ(frameSize.width, firstFrame.cols);
        EXPECT_EQ(frameSize.height, firstFrame.rows);
        cameras.push_back(m_cap);
    }
