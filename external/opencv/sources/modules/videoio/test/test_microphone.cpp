...
TEST(DISABLED_videoio_micro, basic)
{
    ...
    VideoCapture m_cap(0, cv::CAP_MSMF, params);
    ASSERT_TRUE(m_cap.isOpened());

    int samplesPerSecond = (int)m_cap.get(cv::CAP_PROP_AUDIO_SAMPLES_PER_SECOND);
    const int audio_base_index = (int)m_cap.get(cv::CAP_PROP_AUDIO_BASE_INDEX);
    ...
    while ((sysTimeCurr-sysTimePrev)/cvTickFreq < 10)
    {
        if (m_cap.grab())
        {
            ASSERT_TRUE(m_cap.retrieve(frame, audio_base_index));
            sysTimeCurr = cv::getTickCount();
        }
    }
    validSize = samplesPerSecond*(int)((sysTimeCurr-sysTimePrev)/cvTickFreq);
    cursize = (int)m_cap.get(cv::CAP_PROP_AUDIO_POS);
