...
TEST_P(videoio_gstreamer, read_check)
{
    ...
    VideoCapture m_cap;
    ASSERT_NO_THROW(m_cap.open(pipeline.str(), CAP_GSTREAMER));
...
TEST(videoio_gstreamer, timeout_property)
{
    ...
    VideoCapture m_cap;
    m_cap.open("videotestsrc ! appsink", CAP_GSTREAMER);
    ASSERT_TRUE(m_cap.isOpened());
    ...
    EXPECT_NEAR(default_timeout, m_cap.get(CAP_PROP_OPEN_TIMEOUT_MSEC), 1e-3);
    const double current_read_timeout = m_cap.get(CAP_PROP_READ_TIMEOUT_MSEC);
    ...
    m_cap.set(CAP_PROP_OPEN_TIMEOUT_MSEC, open_timeout);
    EXPECT_NEAR(open_timeout, m_cap.get(CAP_PROP_OPEN_TIMEOUT_MSEC), 1e-3);
    if (read_timeout_supported)
    {
        m_cap.set(CAP_PROP_READ_TIMEOUT_MSEC, read_timeout);
        EXPECT_NEAR(read_timeout, m_cap.get(CAP_PROP_READ_TIMEOUT_MSEC), 1e-3);
    }
}
...
TEST_P(gstreamer_bunny, manual_seek)
{
    ...
    VideoCapture m_cap;
    m_cap.open(pipeline, CAP_GSTREAMER);
    ASSERT_TRUE(m_cap.isOpened());
    Mat img;
    for (int i = 0; i < 10; i++)
    {
    ...
    m_cap.set(CAP_PROP_POS_MSEC, target_pos);
    m_cap >> img;
    EXPECT_FALSE(img.empty());
    double actual_pos = m_cap.get(CAP_PROP_POS_MSEC);
