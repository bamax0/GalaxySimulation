...
TEST(videoio_mfx, read_invalid)
{
    ...
    VideoCapture m_cap;
    ASSERT_NO_THROW(m_cap.open("nonexistent-file", CAP_INTEL_MFX));
...
TEST_P(videoio_mfx, read_write_raw)
{
    ...
    // Read video
    VideoCapture m_cap;
    m_cap.open(filename, CAP_INTEL_MFX);
    ASSERT_TRUE(m_cap.isOpened());
    EXPECT_EQ(FRAME_SIZE.width, m_cap.get(CAP_PROP_FRAME_WIDTH));
    EXPECT_EQ(FRAME_SIZE.height, m_cap.get(CAP_PROP_FRAME_HEIGHT));
    double psnrThreshold = (fourcc == VideoWriter::fourcc('M', 'P', 'G', '2')) ? 27.0 : 29.5;  // experimentally chosen value
    for (int i = 0; i < FRAME_COUNT; ++i)
    {
        SCOPED_TRACE(i);
        ASSERT_TRUE(m_cap.read(frame));
