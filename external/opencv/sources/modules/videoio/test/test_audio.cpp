...
public:
    ...
    void doTest()
    {
        ASSERT_TRUE(m_cap.open(findDataFile(root + fileName), backend, params));
        const int audioBaseIndex = static_cast<int>(m_cap.get(cv::CAP_PROP_AUDIO_BASE_INDEX));
        ...
        for (;;)
        {
            if (m_cap.grab())
            {
                ...
                {
                    ASSERT_TRUE(m_cap.retrieve(audioFrame, audioBaseIndex + nCh));
...
public:
    ...
    void doTest()
    {
        ...
        const int samplePerSecond = (int)m_cap.get(CAP_PROP_AUDIO_SAMPLES_PER_SECOND);
        ...
        for (int frame = 0; frame < numberOfFrames; frame++)
        {
            ...
            ASSERT_TRUE(m_cap.grab());
            if (frame == 0)
            {
                double audio_shift = m_cap.get(CAP_PROP_AUDIO_SHIFT_NSEC);
                double video0_timestamp = m_cap.get(CAP_PROP_POS_MSEC) * 1e-3;
            ...
            ASSERT_TRUE(m_cap.retrieve(videoFrame));
            if (epsilon >= 0)
            {
            ...
            if (frame < 5 || frame >= numberOfFrames-5)
                std::cout << "frame=" << frame << ":  audioFrameSize=" << audioFrameCols << "  videoTimestamp=" << m_cap.get(CAP_PROP_POS_MSEC) << " ms" << std::endl;
            else if (frame == 6)
            ...
            if (frame != 0 && frame != numberOfFrames-1)
            {
                ...
                EXPECT_NEAR(
                        m_cap.get(CAP_PROP_AUDIO_POS) / samplePerSecond + audio0_timestamp,
                        m_cap.get(CAP_PROP_POS_MSEC) * 1e-3,
                        (1.0 / fps) * 0.6)
                    << "CAP_PROP_AUDIO_POS=" << m_cap.get(CAP_PROP_AUDIO_POS) << " CAP_PROP_POS_MSEC=" << m_cap.get(CAP_PROP_POS_MSEC);
            }
...
TEST(AudioOpenCheck, bad_arg_invalid_audio_stream)
{
    ...
    VideoCapture m_cap;
    m_cap.open(findDataFile(fileName), cv::CAP_MSMF, params);
    ASSERT_FALSE(m_cap.isOpened());
}
...
TEST(AudioOpenCheck, bad_arg_invalid_audio_sample_per_second)
{
    ...
    VideoCapture m_cap;
    m_cap.open(findDataFile(fileName), cv::CAP_ANY, params);
    ASSERT_FALSE(m_cap.isOpened());
}
