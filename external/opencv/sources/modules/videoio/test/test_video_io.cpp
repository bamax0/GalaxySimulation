...
protected:
    ...
    void checkFrameRead(int idx, VideoCapture & m_cap)
    {
        //int frameID = (int)cap.get(CAP_PROP_POS_FRAMES);
        Mat img;
        ASSERT_NO_THROW(m_cap >> img);
        //std::cout << "idx=" << idx << " img=" << img.size() << " frameID=" << frameID << std::endl;
        ASSERT_FALSE(img.empty()) << "idx=" << idx;
        checkFrameContent(img, idx);
    }
    void checkFrameSeek(int idx, VideoCapture & m_cap)
    {
        bool canSeek = false;
        ASSERT_NO_THROW(canSeek = m_cap.set(CAP_PROP_POS_FRAMES, idx));
        if (!canSeek)
        {
            std::cout << "Seek to frame '" << idx << "' is not supported. SKIP." << std::endl;
            return;
        }
        EXPECT_EQ(idx, (int)m_cap.get(CAP_PROP_POS_FRAMES));
        checkFrameRead(idx, m_cap);
    }
public:
    void doTest()
    {
        ...
        writeVideo();
        VideoCapture m_cap;
        ASSERT_NO_THROW(m_cap.open(video_file, apiPref));
        if (!m_cap.isOpened())
        {
...

        {
            ...
            {
                checkFrameRead(k, m_cap);
                if (::testing::Test::HasFailure() && k % 10 == 0)
                    break;
            }
        }
        bool canSeek = false;
        EXPECT_NO_THROW(canSeek = m_cap.set(CAP_PROP_POS_FRAMES, 0));
        if (!canSeek)
        {
        ...
        if (ext != "wmv" && ext != "h264" && ext != "h265")
        {
            ...
            bool res = false;
            EXPECT_NO_THROW(res = m_cap.set(CAP_PROP_POS_FRAMES, 0));
            ASSERT_TRUE(res);
            for (int k = 0; k < n_frames; k += 20)
            {
                checkFrameSeek(k, m_cap);
                if (::testing::Test::HasFailure() && k % 10 == 0)
                    break;
            }
        }
        ...
        if (ext != "mpg" && ext != "wmv" && ext != "h264" && ext != "h265")
        {
            ...
            for (int k = 0; k < 10; ++k)
            {
                checkFrameSeek(cvtest::TS::ptr()->get_rng().uniform(0, n_frames), m_cap);
                if (::testing::Test::HasFailure() && k % 10 == 0)
                    break;
            }
        }
...
public:
    ...
    void doFrameCountTest()
    {
        ...
        if (cvtest::skipUnstableTests && apiPref == CAP_MSMF && (ext == "h264" || ext == "h265" || ext == "mpg"))
            throw SkipTestException("Unstable MSMF test");
        VideoCapture m_cap;
        EXPECT_NO_THROW(m_cap.open(video_file, apiPref));
        if (!m_cap.isOpened())
        {
            std::cout << "SKIP test: backend " << apiPref << " can't open the video: " << video_file << std::endl;
            return;
        ...
        double fps_prop = 0;
        EXPECT_NO_THROW(fps_prop = m_cap.get(CAP_PROP_FPS));
        if (fps_prop > 0)
        ...
        int count_prop = 0;
        EXPECT_NO_THROW(count_prop = (int)m_cap.get(CAP_PROP_FRAME_COUNT));
    ...
    void doTimestampTest()
    {
        ...
        int frame_count = (int)m_cap.get(CAP_PROP_FRAME_COUNT);
        ...
        for (int i = 0; i < frame_count; i++)
        {
            double timestamp = 0;
            ASSERT_NO_THROW(m_cap >> img);
            EXPECT_NO_THROW(timestamp = m_cap.get(CAP_PROP_POS_MSEC));
...
TEST(Videoio, exceptions)
{
    ...
    EXPECT_FALSE(m_cap.grab());
    EXPECT_FALSE(m_cap.retrieve(mat));
    EXPECT_FALSE(m_cap.set(CAP_PROP_POS_FRAMES, 1));
    EXPECT_FALSE(m_cap.open("this_does_not_exist.avi", CAP_OPENCV_MJPEG));

    m_cap.setExceptionMode(true);

    EXPECT_THROW(m_cap.grab(), Exception);
    EXPECT_THROW(m_cap.retrieve(mat), Exception);
    EXPECT_THROW(m_cap.set(CAP_PROP_POS_FRAMES, 1), Exception);
    EXPECT_THROW(m_cap.open("this_does_not_exist.avi", CAP_OPENCV_MJPEG), Exception);
}
...
TEST_P(safe_capture, frames_independency)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        ASSERT_NO_THROW(m_cap >> frames[i]);
...
TEST_P(stream_capture, read)
{
    ...
    EXPECT_NO_THROW(m_cap.open(makePtr<BufferStream>(video_file), apiPref, {}));
