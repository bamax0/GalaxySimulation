...
TEST(videoio_images, basic_read)
{
    ImageCollection col;
    col.generate(20);
    VideoCapture m_cap(col.getFirstFilename(), CAP_IMAGES);
    ASSERT_TRUE(m_cap.isOpened());
    size_t idx = 0;
    while (m_cap.isOpened()) // TODO: isOpened is always true, even if there are no more images
    {
        Mat img;
        const bool read_res = m_cap.read(img);
        if (!read_res)
            break;
...
TEST(videoio_images, bad)
{
    ImageCollection col;
    {
        ...
        VideoCapture m_cap(buf.str(), CAP_IMAGES);
        EXPECT_FALSE(m_cap.isOpened());
        Mat img;
        EXPECT_FALSE(m_cap.read(img));
    }
}
...
TEST(videoio_images, seek)
{
    ...
    VideoCapture m_cap(col.getFirstFilename(), CAP_IMAGES);
    ASSERT_TRUE(m_cap.isOpened());
    EXPECT_EQ((size_t)count, (size_t)m_cap.get(CAP_PROP_FRAME_COUNT));
    vector<int> positions { count / 2, 0, 1, count - 1, count, count + 100, -1, -100 };
    for (const auto &pos : positions)
    {
        Mat img;
        const bool res = m_cap.set(CAP_PROP_POS_FRAMES, pos);
...
//            EXPECT_FALSE(res); // TODO: backend clamps invalid value to valid range, actual result is 'true'
        ...
        {
            EXPECT_TRUE(res);
            EXPECT_GE(1., m_cap.get(CAP_PROP_POS_AVI_RATIO));
            EXPECT_NEAR((double)pos / (count - 1),  m_cap.get(CAP_PROP_POS_AVI_RATIO), 1e-2);
            EXPECT_EQ(pos, static_cast<decltype(pos)>(m_cap.get(CAP_PROP_POS_FRAMES)));
            EXPECT_TRUE(m_cap.read(img));
...
TEST(videoio_images, pattern_overflow)
{
    ...
    {
        VideoCapture m_cap(col.getPatternFilename(), CAP_IMAGES);
        ASSERT_TRUE(m_cap.isOpened());
        for (size_t idx = col.first_idx; idx <= col.last_idx; ++idx)
        {
            Mat img;
