...
void applyParametersFallback(const Ptr<IVideoCapture>& m_cap, const VideoCaptureParameters& params)
{
    std::vector<int> props = params.getUnused();
    CV_LOG_INFO(NULL, "VIDEOIO: Backend '" << videoio_registry::getBackendName((VideoCaptureAPIs)m_cap->getCaptureDomain()) <<
    ...
    for (int prop : props)
    {
        ...
        if (!m_cap->setProperty(prop, value))
        {
...
public:
    ...
    Ptr<IVideoCapture> createCapture(int camera, const VideoCaptureParameters& params) const CV_OVERRIDE
    {
        ...
        {
            Ptr<IVideoCapture> m_cap = fn_createCaptureCamera_(camera);
            if (m_cap && !params.empty())
            {
                applyParametersFallback(m_cap, params);
            }
            return m_cap;
        }
    ...
    Ptr<IVideoCapture> createCapture(const std::string &filename, const VideoCaptureParameters& params) const CV_OVERRIDE
    {
        ...
        {
            Ptr<IVideoCapture> m_cap = fn_createCaptureFile_(filename);
            if (m_cap && !params.empty())
            {
                applyParametersFallback(m_cap, params);
            }
    ...
    Ptr<IVideoCapture> createCapture(const Ptr<IStreamReader>& stream, const VideoCaptureParameters& params) const CV_OVERRIDE
    {
        ...
        {
            Ptr<IVideoCapture> m_cap = fn_createCaptureStream_(stream);
            if (m_cap && !params.empty())
            {
                applyParametersFallback(m_cap, params);
            }
