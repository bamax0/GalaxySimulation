...
public:
            ...
            const std::string &filename, const Ptr<IStreamReader>& stream, int camera, const VideoCaptureParameters& params)
    {
        ...
        else if (plugin_api->v0.Capture_open)
        {
            ...
            {
                CV_Assert(capture);
                Ptr<PluginCapture> m_cap = makePtr<PluginCapture>(plugin_api, capture);
...
Ptr<IVideoCapture> PluginBackend::createCapture(int camera, const VideoCaptureParameters& params) const
{
    try
    {
        ...
        if (plugin_api_)
        {
            Ptr<IVideoCapture> m_cap = legacy::PluginCapture::create(plugin_api_, std::string(), camera); //.staticCast<IVideoCapture>();
            if (m_cap && !params.empty())
            {
                applyParametersFallback(m_cap, params);
            }
...
Ptr<IVideoCapture> PluginBackend::createCapture(const std::string &filename, const VideoCaptureParameters& params) const
{
    try
    {
        ...
        if (plugin_api_)
        {
            Ptr<IVideoCapture> m_cap = legacy::PluginCapture::create(plugin_api_, filename, 0); //.staticCast<IVideoCapture>();
            if (m_cap && !params.empty())
            {
                applyParametersFallback(m_cap, params);
            }
