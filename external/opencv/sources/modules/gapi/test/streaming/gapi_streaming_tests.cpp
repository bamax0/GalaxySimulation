...
struct GAPI_Streaming: public ::testing::TestWithParam<std::tuple<KernelPackage,
    ...
    GAPI_Streaming() {
        ...
        std::tie(pkg_kind, m_cap) = GetParam();
    ...
    cv::GCompileArgs getCompileArgs() {
        ...
        if (m_cap) {
            args += cv::compile_args(cv::gapi::streaming::queue_capacity{m_cap.value()});
        }
        return args;
    }

    cv::GKernelPackage       pkg;
    cv::optional<size_t>     m_cap;
};
...
TEST(OneVPL_Source, Init)
{
    ...
    cv::Ptr<cv::gapi::wip::IStreamSource> m_cap;
    bool cap_created = false;
    try {
        m_cap = cv::gapi::wip::make_onevpl_src(stream_data_provider, src_params);
        cap_created = true;
