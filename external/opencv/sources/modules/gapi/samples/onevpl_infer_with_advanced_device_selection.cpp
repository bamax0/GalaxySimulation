...
int main(int argc, char *argv[]) {
    ...
    // Create source
    cv::gapi::wip::IStreamSource::Ptr m_cap;
    try {
        if (is_gpu(source_device)) {
            ...
            m_cap = cv::gapi::wip::make_onevpl_src(file_path, source_cfgs,
                                                 gpu_accel_device.value(),
                                                 gpu_accel_ctx.value());
        } else {
            m_cap = cv::gapi::wip::make_onevpl_src(file_path, source_cfgs);
        }
        std::cout << "oneVPL source description: " << m_cap->descr_of() << std::endl;
    } catch (const std::exception& ex) {
        std::cerr << "Cannot create source: " << ex.what() << std::endl;
        return -1;
    }

    cv::GMetaArg descr = m_cap->descr_of();
    auto frame_descr = cv::util::get<cv::GFrameDesc>(descr);
    cv::GOpaque<cv::Rect> in_roi;
    auto inputs = cv::gin(m_cap);
